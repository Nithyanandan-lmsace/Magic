...
// NOTE: We don't upgrade functions 
...
function xmldb_auth_magic_upgrade($oldversion) {
    ...
    // Expiry before notify.
    $field = new xmldb_field('expirybeforenotify', XMLDB_TYPE_TEXT, '', null, null, null, null, 'expiryunassignglobalrole');
    // Conditionally launch add field redirectaftersubmisson.
    if (!$dbman->field_exists($table, $field)) {
        $dbman->add_field($table, $field);
    }
    ...
    // Created auth_magic_revocation_logs.
    $table = new xmldb_table('auth_magic_revocation_logs');
    ...
    // Created auth_magic_revocation_logs.
    $table = new xmldb_table('auth_magic_confirmation_logs');
    ...
    if ($oldversion < 2024102306) {
        ...
        $campaignselfcapabilities = $DB->get_records_menu('role_capabilities', [
            'contextid' => $context->id,
            'capability' => 'auth/magic:campaignself',
        ], '', 'roleid, permission');

        $campaignteamcapabilities = $DB->get_records_menu('role_capabilities', [
            'contextid' => $context->id,
            'capability' => 'auth/magic:campaignteam',
        ], '', 'roleid, permission');

        if (
            key_exists('user', $roles)
            && key_exists($roles['user'], $campaignselfcapabilities)
        ) {
            unassign_capability('auth/magic:campaignself',
                    $roles['user'], $context->id);
        }

        if (
            key_exists('user', $roles)
            && key_exists($roles['user'], $campaignteamcapabilities)
        ) {
            unassign_capability('auth/magic:campaignteam',
                    $roles['user'], $context->id);
        }

        // Videotime savepoint reached.
        upgrade_plugin_savepoint(true, 2024102306, 'auth', 'magic');
    }
