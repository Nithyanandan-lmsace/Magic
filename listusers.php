...
/**
 * Display list of users for magic authentication method.
 *
 * @package    auth_magic
...
$linkexpiry = optional_param('linkexpiry', 0, PARAM_INT);
...
require_login();
$url = new moodle_url('/auth/magic/listusers.php', ['userid' => $userid]);
$PAGE->set_url($url);
...
$strlinkexpiry = get_string('linkexpirytime', 'auth_magic');
$returnparams = ['sort' => $sort, 'dir' => $dir, 'perpage' => $perpage, 'page' => $page];
if ($userid) {
...
} else if ($linkexpiry && confirm_sesskey()) {
    $usercontext = context_user::instance($linkexpiry);
    if ($userid) {
        require_capability('auth/magic:childusersetlinkexpirytime', $usercontext);
    } else {
        require_capability('auth/magic:usersetlinkexpirytime', $sitecontext);
    }
}
// Set manually override magic link expiration time.
$params = ['contextid' => $sitecontext->id];
$PAGE->requires->js_call_amd('auth_magic/linkexpirytime', 'init', [$params]);

echo $OUTPUT->header();

$columns = ['firstname', 'lastname', 'courses', 'email', 'lastaccess'];
...
if (!$users) {
    ...
    foreach ($users as $user) {
        ...
        // Suspend button.
        if (has_capability('auth/magic:usersuspend', $sitecontext) ||
            has_capability('auth/magic:childusersuspend', $usercontext)) {
            if ($user->suspended) {
        ...
        if (has_capability('auth/magic:userupdate', $sitecontext)) {
            ...
            $url = new moodle_url('/user/edit.php', ['id' => $user->id, 'returnto' => 'profile']);
            $buttons[] = html_writer::link($url, $OUTPUT->pix_icon('t/edit', $stredit));
        }
        ...
        if (has_capability('auth/magic:usercopylink', $sitecontext) ||
            ...
            $buttons[] = html_writer::link($returnurl, $OUTPUT->pix_icon('t/copy', $strcopy),
                ['class' => 'magic-invitationlink', 'data-invitationlink' => $invitationlink]);
        }

        // Send link.
        if (has_capability('auth/magic:usersendlink', $sitecontext) ||
            has_capability('auth/magic:childusersendlink', $usercontext)) {
            $url = new moodle_url($returnurl, ['sendlink' => $user->id, 'sesskey' => sesskey()]);
            $buttons[] = html_writer::link($url, $OUTPUT->pix_icon('t/sendmessage', $strsend));
        }

        // Login link expiration time.
        if (has_capability('auth/magic:usersetlinkexpirytime', $sitecontext) ||
            has_capability('auth/magic:childusersetlinkexpirytime', $usercontext)) {
            $buttons[] = html_writer::link($returnurl, $OUTPUT->pix_icon('i/duration', $strlinkexpiry),
                ['class' => 'magic-loginlink_expiry', 'data-id' => $user->id]);
        }
